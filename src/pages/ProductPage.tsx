...
export function ProductPage() {
  ...
  const hasShoeSizes =
    product?.variants.some((v) => v.shoeSize) ?? false;
  ...
  // Ajout automatique si on arrive depuis un "ajout rapide"
  // sur une fiche produit sans variante obligatoire.
  useEffect(() => {
    if (
  ...
  return (
    <div className="container-fitora py-8 md:py-12">
      <nav className="mb-6 flex items-center gap-1.5 text-xs text-fitora-gray-dim">
        <Link
          to="/"
          className="hover:text-fitora-white"
        >
          Accueil
        </Link>
        ...
        <span className="text-fitora-gray">
          {product.name}
        </span>
      </nav>
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          <p className="text-xs uppercase tracking-wide text-fitora-gray">
          ...
          <p className="mt-5 text-sm leading-relaxed text-fitora-gray">
            {product.description}
          </p>
          ...
          {colors.length > 0 && (
            <div className="mt-6">
              <p className="mb-2 text-sm font-semibold">
                Couleur :{" "}
                <span className="font-normal text-fitora-gray">
                  {color}
                </span>
              </p>
            ...
            <div className="mt-8 border-t border-fitora-border pt-6">
              ...
              <ul className="space-y-1.5">
                {product.features.map((f) => (
                  <li
                    key={f}
                    className="flex items-start gap-2 text-sm text-fitora-gray"
                  >
        ...
        <div className="mt-16 border-t border-fitora-border pt-10">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {product.reviews.map((r) => (
              <div
                key={r.id}
                className="rounded-2xl bg-fitora-charcoal p-5"
              >
                ...
                <p className="mt-2 text-sm text-fitora-gray">
                  {r.comment}
                </p>
              </div>
            ))}
