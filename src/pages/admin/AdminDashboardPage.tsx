...
function startOfWeek(d: Date): Date {
  ...
  const day = copy.getDay() === 0 ? 7 : copy.getDay(); // lundi = 1 ... dimanche = 7
...
function sumRevenue(orders: Order[], since: Date): number {
  return orders
...
export function AdminDashboardPage() {
  ...
  const revenueToday = sumRevenue(orders, startOfDay(now));
  const revenueThisWeek = sumRevenue(orders, startOfWeek(now));
  const revenueThisMonth = sumRevenue(orders, startOfMonth(now));
  const revenueThisYear = sumRevenue(orders, startOfYear(now));
  ...
  const outOfStock = products.filter(
    (p) => p.outOfStockOverride || p.variants.every((v) => v.stockAvailable - v.stockReserved <= 0)
  ).length;
  ...
  const yearlySales = useMemo(() => buildYearlySales(orders), [orders]);

  return (
    <div className="space-y-8">
      <div>
        <h2 className="mb-3 font-display text-sm font-semibold text-fitora-gray">
          Chiffre d'affaires
        </h2>
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          <AdminStatCard icon={Wallet} label="Aujourd'hui" value={formatFCFA(revenueToday)} accent />
          <AdminStatCard icon={CalendarDays} label="Cette semaine" value={formatFCFA(revenueThisWeek)} />
          <AdminStatCard icon={CalendarRange} label="Ce mois" value={formatFCFA(revenueThisMonth)} />
          <AdminStatCard icon={Calendar} label="Cette année" value={formatFCFA(revenueThisYear)} />
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <AdminStatCard icon={Wallet} label="CA total (hors annulées)" value={formatFCFA(revenue)} accent />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl bg-fitora-charcoal p-6">
          <h3 className="mb-4 font-display text-sm font-semibold">Ventes mensuelles (12 mois)</h3>
          <SimpleBarChart data={monthlySales} formatValue={(v) => `${v}`} />
        </div>
        <div className="rounded-2xl bg-fitora-charcoal p-6">
          <h3 className="mb-4 font-display text-sm font-semibold">Chiffre d'affaires annuel</h3>
          <SimpleBarChart data={yearlySales} formatValue={(v) => formatFCFA(v)} />
        </div>
      </div>
...
function buildYearlySales(orders: Order[]) {
  const currentYear = new Date().getFullYear();
  const years = Array.from({ length: 4 }).map((_, i) => currentYear - 3 + i);
  return years.map((year) => ({
    label: String(year),
    value: orders
      .filter((o) => o.status !== "cancelled" && new Date(o.createdAt).getFullYear() === year)
      .reduce((sum, o) => sum + o.total, 0),
  }));
}
